...
class SaberRenderOverlay:
    def __init__(self, env_saber: ModularTaskEnv) -> None:
        """Initializes a new `SaberRenderOverlay`."""
    ...
    def _uses_health_status(self) -> bool:
        task_config = getattr(
            self.env_saber.unwrapped.scene["saber_p0_robot"].cfg, "task_cfg", None
        )
        if task_config is None:
            return False
        return bool(getattr(task_config, "use_health_status", False))
    ...
    def _update_target_explosions(self, current_time: float) -> None:
        ...
        uw = self.env_saber.unwrapped.scene["saber_p0_robot"].data
        task_state = self.env_saber.unwrapped._saber_task_state

        active = np.asarray(
            task_state.get("target_pool_active").cpu().numpy(), dtype=bool
        )
        is_obstacle = np.asarray(
            task_state.get("target_pool_is_obstacle").cpu().numpy(), dtype=bool
        )
    ...
    def run(self, frame, enable_target_explosions=False):
        uw = self.env_saber.unwrapped.scene["saber_p0_robot"].data
        task_state = self.env_saber.unwrapped._saber_task_state

        _current_time = float(uw.data.time.cpu().numpy())
        if enable_target_explosions:
            frame = self._draw_target_explosions(frame, _current_time)
        frame = np.array(
            add_text_to_frame(
                frame,
        ...
        frame = np.array(
            add_text_to_frame(
                frame,
                f"Hits: {task_state['target_pool_hits'].cpu().numpy().item()}",
                pos=(5, 30),
                color=(10, 255, 10),
                fontsize=18,
            )
        ...
        frame = np.array(
            add_text_to_frame(
                frame,
                f"Misses: {task_state['target_pool_misses'].cpu().numpy().item()}",
                pos=(5, 50),
                color=(255, 0, 0),
                fontsize=18,
            )
        ...
        if self._uses_health_status():
            _health_status = self._transitioned_health_status(
                _current_time, float(task_state.get("health_status", 0.5))
            )
            frame = _draw_health_bar(frame, _health_status)
        else:

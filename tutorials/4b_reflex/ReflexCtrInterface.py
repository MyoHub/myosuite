...
class MyoLegReflex(object):
    ...
    def get_obs_dict(self):
        ...
        pel_euler = quat2euler(self.env.mj_data.body("pelvis").xquat.copy())
        ...
        temp_seg_vel = self.env.mj_data.object_velocity(
            "pelvis", "body", local_frame=False
        ).copy()
        ...
        pelvis_vel = np.hstack(
            (np.array([dx_local, dy_local, lin_seg_vel[2]]), temp_seg_vel[1])
        )

        # GRF from foot contact sensor values
        # GRF from foot contact sensor values
        temp_right = (
            self.env.mj_data.sensor("r_foot").data[0].copy()
            + self.env.mj_data.sensor("r_toes").data[0].copy()
        )
        temp_left = (
            self.env.mj_data.sensor("l_foot").data[0].copy()
            + self.env.mj_data.sensor("l_toes").data[0].copy()
        )
        ...
        for s_leg, s_legc in zip(["r_leg", "l_leg"], ["l_leg", "r_leg"]):
            ...
            sensor_data[s_leg]["phi_hip"] = (
                np.pi - self.env.mj_data.jnt(f"hip_flexion_{s_leg[0]}").qpos[0].copy()
            )
            sensor_data[s_leg]["phi_knee"] = (
                np.pi - self.env.mj_data.jnt(f"knee_angle_{s_leg[0]}").qpos[0].copy()
            )
            sensor_data[s_leg]["phi_ankle"] = (
                0.5 * np.pi
                - self.env.mj_data.jnt(f"ankle_angle_{s_leg[0]}").qpos[0].copy()
            )
            sensor_data[s_leg]["dphi_knee"] = (
                self.env.mj_data.jnt(f"knee_angle_{s_leg[0]}").qvel[0].copy()
            )
            ...
            dphi_hip = self.env.mj_data.jnt(f"hip_flexion_{s_leg[0]}").qvel[0].copy()
            sensor_data[s_leg]["dalpha"] = (
                dphi_hip - 0.5 * sensor_data[s_leg]["dphi_knee"]
            )

            sensor_data[s_leg]["alpha_f"] = (
                -1 * self.env.mj_data.jnt(f"hip_adduction_{s_leg[0]}").qpos[0].copy()
            ) + 0.5 * np.pi
    ...
    def run_reflex_step(self):
        ...
        temp_pel_euler = quat2euler(self.env.mj_data.body("root").xquat.copy())

        # Check if the simulation is still alive (height of pelvs still above threshold, has not fallen down yet)
        if (
            self.env.mj_data.body("pelvis").xpos[2] < 0.65
    ...
    def _set_muscle_groups(self):
        # ----- Gluteus group -----
        glu_r = [
            self.env.mj_model.actuator("glmax1_r").id,
            self.env.mj_model.actuator("glmax2_r").id,
            self.env.mj_model.actuator("glmax3_r").id,
            self.env.mj_model.actuator("glmed3_r").id,
        ]

        glu_l = [
            self.env.mj_model.actuator("glmax1_l").id,
            self.env.mj_model.actuator("glmax2_l").id,
            self.env.mj_model.actuator("glmax3_l").id,
            self.env.mj_model.actuator("glmed3_l").id,
        ]
        ...
        ham_r = [
            self.env.mj_model.actuator("semimem_r").id,
            self.env.mj_model.actuator("semiten_r").id,
            self.env.mj_model.actuator("bflh_r").id,
        ]

        ham_l = [
            self.env.mj_model.actuator("semimem_l").id,
            self.env.mj_model.actuator("semiten_l").id,
            self.env.mj_model.actuator("bflh_l").id,
        ]
        ...
        bfsh_r = [self.env.mj_model.actuator("bfsh_r").id]

        bfsh_l = [self.env.mj_model.actuator("bfsh_l").id]
        ...
        # ----- Gastrocnemius -----
        gas_r = [
            self.env.mj_model.actuator("gaslat_r").id,
            self.env.mj_model.actuator("gasmed_r").id,
        ]

        gas_l = [
            self.env.mj_model.actuator("gaslat_l").id,
            self.env.mj_model.actuator("gasmed_l").id,
        ]
        ...
        # ----- Soleus -----
        sol_r = [
            self.env.mj_model.actuator("soleus_r").id,
            self.env.mj_model.actuator("perbrev_r").id,
            self.env.mj_model.actuator("perlong_r").id,
            self.env.mj_model.actuator("tibpost_r").id,
        ]

        sol_l = [
            self.env.mj_model.actuator("soleus_l").id,
            self.env.mj_model.actuator("perbrev_l").id,
            self.env.mj_model.actuator("perlong_l").id,
            self.env.mj_model.actuator("tibpost_l").id,
        ]
        ...
        hfl_r = [
            self.env.mj_model.actuator("psoas_r").id,
            self.env.mj_model.actuator("iliacus_r").id,
        ]

        hfl_l = [
            self.env.mj_model.actuator("psoas_l").id,
            self.env.mj_model.actuator("iliacus_l").id,
        ]
        ...
        hab_r = [
            self.env.mj_model.actuator("piri_r").id,
            self.env.mj_model.actuator("sart_r").id,
            self.env.mj_model.actuator("glmed1_r").id,
            self.env.mj_model.actuator("glmed2_r").id,
            self.env.mj_model.actuator("glmin1_r").id,
            self.env.mj_model.actuator("glmin2_r").id,
            self.env.mj_model.actuator("glmin3_r").id,
        ]

        hab_l = [
            self.env.mj_model.actuator("piri_l").id,
            self.env.mj_model.actuator("sart_l").id,
            self.env.mj_model.actuator("glmed1_l").id,
            self.env.mj_model.actuator("glmed2_l").id,
            self.env.mj_model.actuator("glmin1_l").id,
            self.env.mj_model.actuator("glmin2_l").id,
            self.env.mj_model.actuator("glmin3_l").id,
        ]
        ...
        had_r = [
            self.env.mj_model.actuator("addbrev_r").id,
            self.env.mj_model.actuator("addlong_r").id,
            self.env.mj_model.actuator("addmagDist_r").id,
            self.env.mj_model.actuator("addmagIsch_r").id,
            self.env.mj_model.actuator("addmagMid_r").id,
            self.env.mj_model.actuator("addmagProx_r").id,
            self.env.mj_model.actuator("grac_r").id,
        ]

        had_l = [
            self.env.mj_model.actuator("addbrev_l").id,
            self.env.mj_model.actuator("addlong_l").id,
            self.env.mj_model.actuator("addmagDist_l").id,
            self.env.mj_model.actuator("addmagIsch_l").id,
            self.env.mj_model.actuator("addmagMid_l").id,
            self.env.mj_model.actuator("addmagProx_l").id,
            self.env.mj_model.actuator("grac_l").id,
        ]
        ...
        # ----- rectus femoris -----
        rf_r = [self.env.mj_model.actuator("recfem_r").id]

        rf_l = [self.env.mj_model.actuator("recfem_l").id]
        ...
        # ----- Vastius group -----
        vas_r = [
            self.env.mj_model.actuator("vasint_r").id,
            self.env.mj_model.actuator("vaslat_r").id,
            self.env.mj_model.actuator("vasmed_r").id,
        ]

        vas_l = [
            self.env.mj_model.actuator("vasint_l").id,
            self.env.mj_model.actuator("vaslat_l").id,
            self.env.mj_model.actuator("vasmed_l").id,
        ]
        ...
        # ----- tibialis anterior -----
        ta_r = [self.env.mj_model.actuator("tibant_r").id]

        ta_l = [self.env.mj_model.actuator("tibant_l").id]
        ...
        temp_L0 = (
            self.env.mj_model.actuator_lengthrange[:, 0]
            - self.env.mj_model.tendon_lengthspring[:, 0]
    ...
    def _set_initial_pose(self, init_dict):
        ...
        self.init_pelvis = self.env.mj_data.body("pelvis").xpos.copy()
        ...
        for joint_name in init_dict["joint_angles"].keys():
            self.env.mj_data.joint(joint_name).qpos[0] = init_dict["joint_angles"][
                joint_name
            ]
    ...
    def update_footstep(self):
        ...
        r_contact = (
            True
            if (self.env.mj_data.sensor("r_foot").data[0].copy())
            > 0.1 * (np.sum(self.env.mj_model.body_mass) * 9.8)
            else False
        )
        l_contact = (
            True
            if (self.env.mj_data.sensor("l_foot").data[0].copy())
            > 0.1 * (np.sum(self.env.mj_model.body_mass) * 9.8)
            else False
        )

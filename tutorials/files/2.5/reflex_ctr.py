...
class LocoCtrl:
    ...
    def set_control_params_RL(self, s_leg, params):
        ...
        cp["theta_tgt"] = (
            params[cp_map["theta_tgt"]] * 10 * np.pi / 180
        )  # *10*np.pi/180
        cp["c0"] = (
            params[cp_map["c0"]] * 20 * np.pi / 180 + 55 * np.pi / 180
        )  # *20*np.pi/180 +55*np.pi/180
        cp["cv"] = params[cp_map["cv"]] * 2 * np.pi / 180  # *2*np.pi/180
        cp["alpha_delta"] = params[cp_map["alpha_delta"]] * 5 * np.pi / 180
        cp["knee_sw_tgt"] = (
            params[cp_map["knee_sw_tgt"]] * 20 * np.pi / 180 + 120 * np.pi / 180
        )  # *20*np.pi/180 +120*np.pi/180
        cp["knee_tgt"] = (
            params[cp_map["knee_tgt"]] * 15 * np.pi / 180 + 160 * np.pi / 180
        )  # *15*np.pi/180 +160*np.pi/180
        cp["knee_off_st"] = (
            params[cp_map["knee_off_st"]] * 10 * np.pi / 180 + 165 * np.pi / 180
        )  # *10*np.pi/180 +165*np.pi/180
        cp["ankle_tgt"] = (
            params[cp_map["ankle_tgt"]] * 20 * np.pi / 180 + 60 * np.pi / 180
        ...
        cp["HFL_6_DG"] = params[cp_map["HFL_6_DG"]] * 0.1
        ...
        cp["GLU_10_PG"] = params[cp_map["GLU_10_PG"]] * 0.5
        ...
        cp["VAS_10_PG"] = params[cp_map["VAS_10_PG"]] * 0.3
    ...
    def _brain_control(self, sensor_data=0):
        ...
        for s_leg in ["r_leg", "l_leg"]:
            if self.control_dimension == 3:
                ...
                self.brain_command[s_leg]["alpha_tgt_f"] = (
                    alpha_tgt_global_frontal - theta_f
                )
            ...
            self.brain_command[s_leg]["hip_tgt"] = (
                self.brain_command[s_leg]["alpha_tgt"]
                + 0.5 * self.brain_command[s_leg]["knee_tgt"]
            )
        ...
        if (
            sensor_data["r_leg"]["contact_ipsi"]
            and sensor_data["l_leg"]["contact_ipsi"]
        ):
            r_delta_alpha = (
                sensor_data["r_leg"]["alpha"] - self.brain_command["r_leg"]["alpha_tgt"]
            )
            l_delta_alpha = (
                sensor_data["l_leg"]["alpha"] - self.brain_command["l_leg"]["alpha_tgt"]
            )
    ...
    def _update_spinal_control_phase(self, s_leg, sensor_data):
        ...
        if self.spinal_control_phase[s_leg]["ph_st"]:
            ...
            # initiate swing
            self.spinal_control_phase[s_leg]["ph_st_sw0"] = self.brain_command[s_leg][
                "swing_init"
            ]
            # do not initiate swing
            self.spinal_control_phase[s_leg][
                "ph_st_st"
            ] = not self.spinal_control_phase[s_leg]["ph_st_sw0"]
        ...
        if self.spinal_control_phase[s_leg]["ph_sw"]:
            ...
            if self.spinal_control_phase[s_leg]["ph_sw_flex_k"]:
                ...
                if (
                    self.spinal_control_phase[s_leg]["ph_sw_stop_l"]
                    and s_l["dalpha"] > 0
                ):  # leg started to retract
                    # hold leg
    ...
    def spinal_control_leg(self, s_leg, sensor_data):
        ...
        self.spinal_control_phase[s_leg]["ph_st_csw"]
        ...
        if self.control_dimension == 3:
            ...
            S_HAB_6 = (ph_st_sw0 * s_l["load_contra"] + ph_sw) * np.maximum(
                cp["HAB_6_PG"] * (s_l["alpha_f"] - alpha_tgt_f), 0
            )
            ...
            S_HAD_6 = (ph_st_sw0 * s_l["load_contra"] + ph_sw) * np.maximum(
                -cp["HAD_6_PG"] * (s_l["alpha_f"] - alpha_tgt_f), 0
            )
        ...
        S_HFL_3 = (
            ...
            * s_l["load_ipsi"]
            * np.maximum(
                -cp["HFL_3_PG"] * (theta - theta_tgt) - cp["HFL_3_DG"] * dtheta, 0
            )
        )
        ...
        S_HFL_10 = ph_sw_hold_l * np.maximum(
            cp["HFL_10_PG"] * (s_l["phi_hip"] - hip_tgt), 0
        )
        ...
        S_GLU_3 = (
            ...
            * s_l["load_ipsi"]
            * np.maximum(
                cp["GLU_3_PG"] * (theta - theta_tgt) + cp["GLU_3_DG"] * dtheta, 0
            )
        )
        ...
        S_BFSH_9 = np.maximum(cp["BFSH_9_G_HAM"] * (S_HAM_9 - cp["BFSH_9_HAM0"]), 0)
        S_BFSH_10 = ph_sw_hold_l * np.maximum(
            cp["BFSH_10_PG"] * (s_l["phi_knee"] - knee_tgt), 0
        )
        stim["BFSH"] = pre_stim + S_BFSH_2 + S_BFSH_7 + S_BFSH_8 + S_BFSH_9 + S_BFSH_10

        S_GAS_2 = ph_st * np.maximum(cp["GAS_2_FG"] * s_l["F_GAS"], 0)
        stim["GAS"] = pre_stim + S_GAS_2
        S_SOL_1 = ph_st * np.maximum(cp["SOL_1_FG"] * s_l["F_SOL"], 0)
        stim["SOL"] = pre_stim + S_SOL_1

        S_TA_5 = np.maximum(cp["TA_5_PG"] * (s_l["phi_ankle"] - ankle_tgt), 0)
        S_TA_5_st = -ph_st * np.maximum(cp["TA_5_G_SOL"] * S_SOL_1, 0)

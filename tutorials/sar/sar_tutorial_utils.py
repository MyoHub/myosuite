...
def show_video(video_path, video_width=600):
    """Displays any mp4 video within the notebook (SAR default width 600).

    Delegates to :func:`myosuite.utils.video_io.show_video`.
    """
    return _video_show_video(video_path, video_width=video_width)
...
class SaveSuccesses(BaseCallback):
    ...
    def _on_rollout_end(self) -> None:
        ...
        if len(self.success_buffer) > 0:
            self.success_results.append(
                sum(self.success_buffer) / len(self.success_buffer)
            )
    ...
    def _on_training_end(self) -> None:
        np.save(
            os.path.join(self.log_dir, f"success_{self.env_name}"),
            np.array(self.success_results),
        )
...
def plot_results(smoothing=1000, experiment="locomotion", terrain=None):
    ...
    if not isinstance(smoothing, int) or smoothing < 1:
        raise ValueError(
            "The smoothing value must be an integer greater than or equal to 1"
        )
...
class SynergyWrapper(gym.ActionWrapper):
    ...
    def __init__(self, env, ica, pca, scaler):
        super().__init__(env)
        self.ica = ica
        self.pca = pca
        self.scaler = scaler
    ...
    for i, __ in tqdm(enumerate(range(episodes))):
        ...
        while not (terminated or truncated):
            ...
            if len(step_out) == 5:
                ...
                obs, r, done, *rest = step_out
                terminated = bool(done)
                truncated = False
                info = rest[-1] if rest else {}
            is_solved.append(info.get("solved", False))

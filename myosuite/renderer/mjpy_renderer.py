...
class MjPyRenderer(Renderer):
    ...
    def render_offscreen(
        ...
        if not self._offscreen_renderer:
            self._offscreen_renderer = mujoco_py.MjRenderContextOffscreen(
                self._sim, device_id=device_id
            )
        ...
        # camera_id = self._mj_model.camera_name2id(camera_id)

        # return self._offscreen_renderer.read_pixels(width, height)
        return self._offscreen_renderer.read_pixels(

...
# -----------------------------------------------------------------------------------------------------------------
    def get_obs_dict(self):
        ...
        pel_euler = R.from_quat(self.env.sim.data.get_body_xquat('pelvis').copy()).as_euler('xyz', degrees=False) # Yaw, Pitch, Roll
        pelvis_roll = pel_euler[2] - (np.pi/2)
        pelvis_pitch = pel_euler[1] * (-1)
        pelvis_yaw = pel_euler[0] + (np.pi)
        ...
        sensor_data['body']['dtheta'] = [-1*pelvis_vel[3], # around local x axis
                                        -1*pelvis_vel[4]] # around local y axis
...
# Integration of code with Myosuite control codes
    ...
    def run_reflex_step(self):
        ...
        # Returns : obs_dict, reward, done_flag
        is_done = False
        ...
        out_dict = self.get_obs_dict() # self.collectData()

        temp_pel_euler = R.from_quat(self.env.sim.data.get_body_xquat('root').copy()).as_euler('xyz', degrees=False) # Yaw, Pitch, Roll
    ...
    def _set_initial_pose(self, init_dict):
        # - Height, in meters to set the height of the model : float
        # - init_dict, holds the initial pose, joint angles, and starting velocities : dict
        #   - model_pose, in radans, in format of a dictionary : dict - {yaw : , pitch: , roll: , joint angles}
        #       *** IMPT NOTE *** This modifies the euler angles of the free root joint
        #   - joint_angles, in radians, in format of dict, keys are to be in Mujoco joint format
        # - condition, '2D' or '3D'
        ...
        temp_quat = R.from_euler('xyz', [init_dict['model_pose']['yaw'], 
                                         init_dict['model_pose']['pitch'], 
                                         init_dict['model_pose']['roll']], degrees=False) # Yaw, Pitch, Roll
        #print(temp_quat.as_quat())

        self.env.sim.data.qpos[3] = temp_quat.as_quat()[0] # Setting no roll, pitch and yaw
        self.env.sim.data.qpos[4] = temp_quat.as_quat()[1]
        self.env.sim.data.qpos[5] = temp_quat.as_quat()[2]
        self.env.sim.data.qpos[6] = temp_quat.as_quat()[3]
        ...
        # Mappings from Matlab code to Python
        # phi0 -> free root joint
        # Lphi120 -> Left ankle joint angle
        # Lphi230 -> left knee joint angle
        # Lphi340 -> Left hip joint angle

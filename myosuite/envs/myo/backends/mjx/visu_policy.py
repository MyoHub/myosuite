...
ppo_network = ppo_networks.make_ppo_networks(
    10, 6, policy_hidden_layer_sizes=[50, 50, 50], preprocess_observations_fn=normalize
)
...
def get_obs(model, data, target):
    ...
    return np.concatenate(
        [[0], position, data.qvel * model.opt.timestep, data.act, target - position]
    )
...
def arm_control(model, data):
    ...
    observations = {
        "state": np.array([get_obs(model, data, data.ctrl[-1])], dtype=np.float16)
    }

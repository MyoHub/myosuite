...
3. **Verify MJX runs on CUDA** (when using `JAX_PLATFORMS=cuda`):
   ```bash
   ...
   device = next(iter(state.data.qpos.devices()))
   on_cuda = device.platform == 'cuda'
   print(f'MJX device: {device}, running on CUDA: {on_cuda}')
   "
   ```

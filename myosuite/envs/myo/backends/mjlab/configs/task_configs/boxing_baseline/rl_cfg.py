...
class BetaDistribution(Distribution):
    ...
    def mean(self) -> torch.Tensor:
        ...
        return (
            self._alpha / (self._alpha + self._beta)
        ) * self._range_scale + self._range_offset  # type: ignore
    ...
    def log_prob(self, outputs: torch.Tensor) -> torch.Tensor:
        ...
        return (self._distribution.log_prob(unscaled) - self._log_range_scale).sum(
            dim=-1
        )  # type: ignore
    ...
    def kl_divergence(
        ...
        return torch.distributions.kl_divergence(
            Beta(old_alpha, old_beta), Beta(new_alpha, new_beta)
        ).sum(dim=-1)

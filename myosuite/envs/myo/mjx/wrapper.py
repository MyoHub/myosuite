...
class MadronaWrapper:
    ...
    def __init__(
        ...
        | (Callable[[mjx.Model], tuple[mjx.Model, mjx.Model]]) = None,
    ):
...
class EpisodeWrapper(Wrapper):
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        for metric_name in state.metrics.keys():
            if (metric_name != "reward") and (
                metric_name not in self.non_accumulation_metrics
            ):
                state.info["episode_metrics"][metric_name] += state.metrics[metric_name]
                state.info["episode_metrics"][metric_name] *= 1 - prev_done
            if metric_name in self.non_accumulation_metrics:
                print(
                    f"Setting non-accumulation metric: {metric_name} to the current value"
                )
                state.info["episode_metrics"][metric_name] = state.metrics[metric_name]
...
def wrap_for_brax_training(
    ...
    if vision:
        env = MadronaWrapper(
            env, num_worlds=num_vision_envs, randomization_fn=randomization_fn
        )
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        def reset_state(current_state):
            ...
            new_state = new_state.replace(
                done=current_state.done, metrics=current_state.metrics
            )
        ...
        def compute_next_state(current_state):
            return jax.lax.cond(
                current_state.done, reset_state, return_state_as_it_is, current_state
            )
...
class AutoResetWrapper(Wrapper):
    ...
    def step(self, state: mjx_env.State, action: jax.Array):
        ...
        data = jax.tree.map(
            where_done,
            state_after_reset.data,
            state.data,  # state.pipeline_state
        )
...
class EvalVmapWrapper(Wrapper):
    ...
    def __init__(
        ...
        predefined_evals: bool | None = True,
    ):
        super().__init__(env)
...
def _maybe_wrap_env_for_evaluation(eval_env, seed):
    ...
    if not hasattr(eval_env, "eval_wrapped"):
        eval_env = EvalVmapWrapper(
            eval_env,
            n_randomizations=n_randomizations,
            predefined_evals=predefined_evals,
        )

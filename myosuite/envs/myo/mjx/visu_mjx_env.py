...
def main(env_name, load_policy, use_reset):
    ...
    spec.add_sensor(
        name="reward+target",
        type=mujoco.mjtSensor.mjSENS_USER,
        dim=1 + env.mjx_model.nq,
    )
    m = spec.compile()
        ...
        while viewer.is_running():
            ...
            state = state.replace(
                data=state.data.replace(
                    mocap_pos=d.mocap_pos, xfrc_applied=d.xfrc_applied
                )
            )
            ...
            d.sensordata[1:] = (
                ...
                if "target_angles" in state.info
                else state.data.qpos
            )

            if load_policy:
                d.ctrl = (
                    state.data.ctrl
                )  # If we are using the policy, we'll visualize the excitation.
            ...
            mujoco.mj_forward(
                m, d
            )  # We only do forward step for the kinematics, no need to integrate.

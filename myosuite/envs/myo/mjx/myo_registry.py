...
def config_create_variant(
    env_config: Callable[[], config_dict.ConfigDict], **kwargs
) -> Callable[[], config_dict.ConfigDict]:
...
def register_environment_with_variants(
    ...
    register_environment(env_name=env_name, env_class=env_class, cfg_class=cfg_class)
...
def get_domain_randomizer(
    env_name: str,
) -> Callable[[mjx.Model, jax.Array], tuple[mjx.Model, mjx.Model]] | None:

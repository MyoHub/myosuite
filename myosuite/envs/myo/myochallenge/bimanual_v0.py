...
class BimanualEnvV1(BaseV0):
    ...
    def _setup(
        ...
        start_center=np.array(
            [-0.4, -0.25, 1.05]
        ),  # Start and goal centers, pos = center + shift * [0, 1]
        ...
        **kwargs,
        ...
        self.id_info = IdInfo(self.mj_model)
        ...
        self.init_obj_z = self.mj_data.site_xpos[self.obj_sid][-1]
        self.target_z = 0.2
        ...
        self.mj_model.body_pos[self.start_bid] = self.start_pos
        self.mj_model.body_pos[self.goal_bid] = self.goal_pos
        ...
        self.obj_mass_range = (
            {
                "low": self.mj_model.body_mass[self.obj_bid] + obj_mass_change[0],
                "high": self.mj_model.body_mass[self.obj_bid] + obj_mass_change[1],
            }
            if obj_mass_change
            else None
        )
        ...
        self.obj_friction_range = (
            {
                "low": self.mj_model.geom_friction[self.obj_gid] - obj_friction_change,
                "high": self.mj_model.geom_friction[self.obj_gid] + obj_friction_change,
            }
        ...
        self.init_qpos[:] = self.mj_model.key_qpos[2].copy()
    ...
    def __center_box_mesh(self):
        ...
        self.mj_model.mesh_vert[self.obj_vert_addr] = r.apply(
            self.mj_model.mesh_vert[self.obj_vert_addr]
        )
        self.mj_model.mesh_normal[self.obj_vert_addr] = r.apply(
            self.mj_model.mesh_normal[self.obj_vert_addr]
        )
        self.mj_model.geom(self.obj_gid - 1).quat = [1, 0, 0, 0]
        self.mj_model.mesh_vert[self.obj_vert_addr] += (
            self.mj_model.geom(self.obj_gid - 1).pos
            - self.mj_model.geom(self.obj_gid).pos
        )[None, :]

        self.mj_model.geom(self.obj_gid - 1).pos = self.mj_model.geom(self.obj_gid).pos
        self.mesh_vert0 = self.mj_model.mesh_vert[self.obj_vert_addr].copy()
    ...
    def get_obs_dict(self, mj_model, mj_data):
        ...
        obs_dict["elbow_fle"] = self.mj_data.joint("elbow_flexion").qpos.copy()
        ...
        touching_objects = set(
            get_touching_objects(this_model, this_data, self.id_info)
        )
        ...
        obs_dict["MPL_ori"] = mat2euler(
            np.reshape(self.mj_data.site_xmat[self.Rpalm1_sid], (3, 3))
        )
    ...
    def step(self, a, **kwargs):
        ...
        if self.normalize_act:
            ...
            processed_controls[robotic_act_ind] = (
                np.mean(self.mj_model.actuator_ctrlrange[robotic_act_ind], axis=-1)
                + processed_controls[robotic_act_ind]
                * (
                    self.mj_model.actuator_ctrlrange[robotic_act_ind, 1]
                    - self.mj_model.actuator_ctrlrange[robotic_act_ind, 0]
                )
                / 2.0
            )
    ...
    def get_metrics(self, paths, successful_steps=5):
        ...
        for path in paths:
            ...
            if (
                np.sum(path["env_infos"]["rwd_dict"]["solved"] * 1.0) > successful_steps
                and evaluate_contact_trajectory(path["env_infos"]["touch_history"])
                is None
            ):
                num_success += 1
        ...
        max_force = np.mean(
            [np.round(p["env_infos"]["obs_dict"]["max_force"][-1], 5) for p in paths]
        )
        goal_dist = np.mean(
            [np.mean(p["env_infos"]["rwd_dict"]["goal_dist"]) for p in paths]
        )

        # average activations over entire trajectory (can be shorter than horizon, if done) realized
        effort = 1.0 * np.mean(
            [np.mean(p["env_infos"]["rwd_dict"]["act"]) for p in paths]
        )
    ...
    def reset(self, **kwargs):
        ...
        if self.obj_mass_range:
            self.mj_model.body_mass[self.obj_bid] = self.np_random.uniform(
                **self.obj_mass_range
        ...
        if self.obj_friction_range:
            self.mj_model.geom_friction[self.obj_gid] = self.np_random.uniform(
                **self.obj_friction_range
            )
        ...
        object_qpos_adr = self.mj_model.body(self.obj_bid).jntadr[0]
        self.mj_data.qpos[object_qpos_adr : object_qpos_adr + 3] = (
...
class IdInfo:
    def __init__(self, model: mujoco.MjModel):
        ...
        myo_bodies = [
            ...
            and model.body(i).name not in ["start", "goal", "manip_object"]
        ]
        ...
        prosth_bodies = [
            model.body(i).id
            for i in range(model.nbody)
            if model.body(i).name.startswith("prosthesis/")
        ]

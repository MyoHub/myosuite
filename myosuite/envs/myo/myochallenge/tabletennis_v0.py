...
class TableTennisEnvV0(BaseV0):
    ...
    def _setup(
        ...
        **kwargs,
        ...
        self.ball_dofadr = self.mj_model.body_dofadr[self.id_info.ball_bid]
    ...
    def get_obs_dict(self, mj_model, mj_data):
        ...
        obs_dict["ball_pos"] = mj_data.site_xpos[self.id_info.ball_sid]
        obs_dict["ball_vel"] = self.get_sensor_by_name(
            mj_model, mj_data, "pingpong_vel_sensor"
        )

        obs_dict["paddle_pos"] = mj_data.site_xpos[self.id_info.paddle_sid]
    ...
    def ref_traj(self, traj_path=r"your_h5.h5"):
        ...
        if not hasattr(self, "_ref_traj_cache"):
            ...
            self._ref_dt = self.mj_model.opt.timestep
        ...
        def _get_ref():
            ...
            if idx < self._ref_traj_cache["T"]:
                ...
                qpos_err = qpos_ref - self.mj_data.qpos
                qvel_err = qvel_ref - self.mj_data.qvel
            else:
    ...
    def reset(self, reset_qpos=None, reset_qvel=None, **kwargs):
        ...
        if self.qpos_noise_range is not None:
            ...
            for j, adr in enumerate(self.mj_model.jnt_qposadr[:-2]):
    ...
    def relaunch_ball(self):
        ...
        ball_vel = self.init_qvel[
            self.ball_dofadr : self.ball_dofadr + 6
        ]  # 6 dof to reset spin
        if self.ball_xyz_range is not None:
            ball_pos = self.np_random.uniform(**self.ball_xyz_range)
            self.mj_model.body_pos[self.id_info.ball_bid] = ball_pos
        ...
        self.mj_data.qpos[self.ball_posadr : self.ball_posadr + 3] = ball_pos
        self.mj_data.qvel[self.ball_dofadr : self.ball_dofadr + 6] = ball_vel
...
class IdInfo:
    def __init__(self, model: mujoco.MjModel):
        ...
        myo_bodies = [
            ...
            and model.body(i).name not in ["pingpong"]
        ]

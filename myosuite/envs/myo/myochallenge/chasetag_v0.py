...
class RepellerChallengeOpponent(ChallengeOpponent):
    ...
    def repeller_stochastic(self):
        ...
        distance = np.array(
            [np.linalg.norm(diff) for diff in (obstacle_pos - opponent_pos[0:2])]
        )
        ...
        escape_direction = (
            escape_direction + 1.57
        )  # Account for rotation in world frame
    ...
    def _calc_angular_vel(self, current_pos, desired_pos):
        ...
        direction_clock = np.abs(0 - current_pos) + (
            2 * np.pi - desired_pos
        )  # Clockwise rotation
        direction_anticlock = (2 * np.pi - current_pos) + (
            0 + desired_pos
...
class ChaseTagEnvV0(WalkEnvV0):
    ...
    def assert_settings(self):
        ...
        ), f"Chase velocity range should be positive. {self.opponent.chase_vel_range}"
        # others assert that range end is bigger than range start
        assert (
            self.opponent.chase_vel_range[0] <= self.opponent.chase_vel_range[1]
        ), f"Chase velocity range is not valid. {self.opponent.chase_vel_range}"
        assert (
            self.opponent.random_vel_range[0] <= self.opponent.random_vel_range[1]
        ), f"Random movement velocity range is not valid {self.opponent.random_vel_range}"
        ...
        if self.repeller_opponent == True:
            ...
            assert (
                len(self.opponent.opponent_probabilities) == 3
            ), "Standard opponent requires 3 probabilities"

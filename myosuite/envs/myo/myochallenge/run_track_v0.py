...
class RunTrack(WalkEnvV0):
        ...
        **kwargs,
        ...
        if init_pose_path is not None:
            ...
            self.init_lookup = self.generate_init_lookup(
                keys=np.arange(48), value="e_swing"
            )
            self.init_lookup = self.generate_init_lookup(
                keys=np.arange(48, 99), value="l_swing", existing_dict=self.init_lookup
            )
            self.init_lookup = self.generate_init_lookup(
                keys=np.arange(99, 183),
                value="e_stance",
                existing_dict=self.init_lookup,
            )
            self.init_lookup = self.generate_init_lookup(
                keys=np.arange(183, 247),
                value="l_stance",
                existing_dict=self.init_lookup,
            )
        ...
        self.OSL_CTRL = MyoOSLController(
            np.sum(self.mj_model.body_mass),
            init_state="e_stance",
            n_sets=self.osl_param_set,
        )
        self.OSL_CTRL.start()

        self.muscle_space = self.mj_model.na  # muscles only
        self.full_ctrl_space = self.mj_model.nu  # Muscles + actuators
        ...
        self.init_qpos[:] = self.mj_model.keyframe("stand").qpos.copy()
        self.init_qvel[:] = 0.0
        self.startFlag = True
    ...
    def step(self, a, **kwargs):
        myoosl_a = self._append_osl_actions(
            mus_actions=a, is_normalized=self.normalize_act
        )
    ...
    def reset(self, OSL_params=None, **kwargs):
        ...
        if self.reset_type != "init":
            ...
            obs = super(WalkEnvV0, self).reset(
                reset_qpos=new_qpos, reset_qvel=new_qvel, **kwargs
            )
            mujoco.mj_forward(self.mj_model, self.mj_data)
        self.OSL_CTRL.start()
    ...
    def _get_randomized_initial_state(self):
        ...
        qpos = self.mj_model.key_qpos[rndInt].copy()
        qvel = self.mj_model.key_qvel[rndInt].copy()
        ...
        qpos[0] = self.np_random.uniform(
            -self.real_width * 0.8, self.real_width * 0.8, size=1
        ).squeeze()
    ...
    def _get_fallen_condition(self):
        ...
        foot_r = self.mj_data.body("osl_foot_assembly").xpos
    ...
    def get_pain(self):
        ...
        for joint in self.pain_jnt:
            pain_score += (
                np.clip(np.abs(self.get_limitfrc(joint).squeeze()), -1000, 1000) / 1000
            )
    ...
    def _get_socket_force(self):
        return self.mj_data.sensor("r_socket_load").data.copy()
    ...
    def get_limitfrc(self, joint_name):
        ...
        return joint_force[self.mj_model.joint(joint_name).dofadr]
    ...
    def get_internal_qpos(self):
        ...
        """
        temp_qpos = np.zeros(
            len(self.biological_jnt),
        )
        counter = 0
        for jnt in self.biological_jnt:
            temp_qpos[counter] = self.mj_data.joint(jnt).qpos[0].copy()
            counter += 1
        return temp_qpos
    ...
    def get_internal_qvel(self):
        ...
        counter = 0
        for jnt in self.biological_jnt:
            temp_qvel[counter] = self.mj_data.joint(jnt).qvel[0].copy()
            counter += 1
    ...
    def muscle_lengths(self):
        ...
        """
        temp_len = np.zeros(
            len(self.biological_act),
        )
        counter = 0
        for jnt in self.biological_act:
            temp_len[counter] = self.mj_data.actuator(jnt).length[0].copy()
            counter += 1
        return temp_len
    ...
    def muscle_forces(self):
        ...
        counter = 0
        for jnt in self.biological_act:
            temp_frc[counter] = self.mj_data.actuator(jnt).force[0].copy()
            counter += 1
    ...
    def muscle_velocities(self):
        ...
        counter = 0
        for jnt in self.biological_act:
            temp_vel[counter] = self.mj_data.actuator(jnt).velocity[0].copy()
            counter += 1
    ...
    def _get_actuator_params(self):
        ...
        """
        actuators = [
            "osl_knee_torque_actuator",
            "osl_ankle_torque_actuator",
        ]

        for actu in actuators:
            self.ACTUATOR_PARAM[actu] = {}
            self.ACTUATOR_PARAM[actu]["id"] = self.mj_data.actuator(actu).id
            self.ACTUATOR_PARAM[actu]["Fmax"] = (
                np.max(self.mj_model.actuator(actu).ctrlrange)
                * self.mj_model.actuator(actu).gear[0]
            )
    ...
    def initializeFromData(self):
        ...
        for joint in self.gait_cycle_headers.keys():
            if joint not in [
                ...
                self.init_qpos[self.mj_model.joint(joint).qposadr[0]] = self.INIT_DATA[
                    start_idx, self.gait_cycle_headers[joint]
                ]

        # Get the Yaw from the init pose
        default_quat = self.init_qpos[
            3:7
        ].copy()  # Get the default facing direction first
        ...
        world_vel_X, world_vel_Y = self.rotate_frame(
        ...
        self.init_qvel[2] = self.INIT_DATA[
    ...
    def adjust_model_height(self):
        ...
        curr_qpos = self.mj_data.qpos.copy()
        curr_qvel = self.mj_data.qvel.copy()

        temp_sens_height = 100
        for sens_site in ["r_heel_btm", "r_toe_btm", "l_heel_btm", "l_toe_btm"]:
            if temp_sens_height > self.mj_data.site(sens_site).xpos[2]:
                temp_sens_height = self.mj_data.site(sens_site).xpos[2].copy()
    ...
    def _append_osl_actions(self, mus_actions, is_normalized):
        ...
        full_actions[0 : self.mj_model.na] = mus_actions[0 : self.mj_model.na].copy()
        ...
        for jnt in ["knee", "ankle"]:
            osl_id = self.mj_model.actuator(f"osl_{jnt}_torque_actuator").id
            osl_ctrl = (
                osl_torque[jnt]
                / self.mj_model.actuator(f"osl_{jnt}_torque_actuator").gear[0]
            )

            # clip for control limits
            min_ctrl = self.mj_model.actuator(f"osl_{jnt}_torque_actuator").ctrlrange[0]
            max_ctrl = self.mj_model.actuator(f"osl_{jnt}_torque_actuator").ctrlrange[1]
    ...
    def get_osl_sens(self):
        ...
        osl_sens_data = {}
        osl_sens_data["knee_angle"] = (
            self.mj_data.joint("osl_knee_angle_r").qpos[0].copy()
        )
        osl_sens_data["knee_vel"] = (
            self.mj_data.joint("osl_knee_angle_r").qvel[0].copy()
        )
        osl_sens_data["ankle_angle"] = (
            self.mj_data.joint("osl_ankle_angle_r").qpos[0].copy()
        )
        osl_sens_data["ankle_vel"] = (
            self.mj_data.joint("osl_ankle_angle_r").qvel[0].copy()
        )
        osl_sens_data["load"] = (
            -1 * self.mj_data.sensor("r_osl_load").data[1].copy()
        )  # Only vertical

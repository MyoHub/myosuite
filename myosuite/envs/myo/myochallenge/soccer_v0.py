...
class GoalKeeper:
    ...
    def sample_goalkeeper_policy(self):
        ...
        if rand_num < self.goalkeeper_probabilities[0]:
            ...
            rand_num
            < self.goalkeeper_probabilities[0] + self.goalkeeper_probabilities[1]
        ):
            self.goalkeeper_policy = "random"
        elif (
            rand_num
            < self.goalkeeper_probabilities[0]
            + self.goalkeeper_probabilities[1]
            + self.goalkeeper_probabilities[2]
        ):
    ...
    def update_goalkeeper_state(self):
        ...
        if self.goalkeeper_policy == "stationary":
            ...
            raise NotImplementedError(
                f"This goalkeeper policy doesn't exist. Chose: stationary, random or block_ball. Policy was: {self.goalkeeper_policy}"
            )
...
class SoccerEnvV0(WalkEnvV0):
    ...
    def _setup(
        ...
        **kwargs,
        ...
        self.goalkeeper.dt = self.mj_model.opt.timestep * self.frame_skip
    ...
    def get_obs_dict(self, mj_model, mj_data):
        ...
        obs_dict["torso_angle"] = self.mj_data.body("torso").xquat.copy()
        obs_dict["pelvis_angle"] = self.mj_data.body("pelvis").xquat.copy()

        obs_dict["muscle_length"] = self.muscle_lengths()
        obs_dict["muscle_velocity"] = self.muscle_velocities()
        obs_dict["muscle_force"] = self.muscle_forces()

        obs_dict["r_toe_pos"] = self.mj_data.geom("r_bofoot").xpos.copy()
        obs_dict["l_toe_pos"] = self.mj_data.geom("l_bofoot").xpos.copy()
    ...
    def get_reward_dict(self, obs_dict):
        ...
        # Success Indicator
        # self.mj_model.site_rgba[self.success_indicator_sid, :] = np.array([0, 2, 0, 0.2]) if rwd_dict['solved'] else np.array([2, 0, 0, 0])
        return rwd_dict
    ...
    def get_jnt_limit_violation(self):
        ...
        for joint in self.JNT_OVEREXT:
            limit_score += (
                np.abs(np.clip(self.get_limitfrc(joint).squeeze(), -1000, 1000)) / 1000
            )
    ...
    def _get_joint_qpos(self):
        ...
        """
        return np.array(
            [self.mj_data.joint(jnt).qpos[0].copy() for jnt in self.myo_joints]
        )
    ...
    def _get_joint_qvel(self):
        ...
        """
        return np.array(
            [self.mj_data.joint(jnt).qvel[0].copy() for jnt in self.myo_joints]
        )

...
class RelocateEnvV0(BaseV0):
    ...
    def get_obs_dict(self, mj_model, mj_data):
        ...
        obs_dict["hand_qpos_corrected"] = mj_data.qpos[:-6].copy()
    ...
    def get_reward_dict(self, obs_dict):
        ...
        self.mj_model.site_size[self.success_indicator_sid, :] = (
            np.array(
                [
                    0.25,
                ]
    ...
    def reset(self, reset_qpos=None, reset_qvel=None, **kwargs):
        ...
        self.mj_model.body_quat[self.goal_bid] = euler2quat(
            self.np_random.uniform(**self.target_rxryrz_range)
        )

        if self.obj_xyz_range is not None:
            self.mj_model.body_pos[self.object_bid] = self.np_random.uniform(
                **self.obj_xyz_range
            )
        ...
        if self.obj_geom_range is not None:
            for body in [
                ...
                for gid in range(self.mj_model.body_geomnum[bid]):
                    ...
                    self.mj_model.geom_type[gid] = self.np_random.choice(
                        [2, 3, 4, 5, 6]
                    )  # random shape
                    self.mj_model.geom_size[gid] = self.np_random.uniform(
                        low=self.obj_geom_range["low"], high=self.obj_geom_range["high"]
                    )  # random size
                    self.mj_model.geom_aabb[gid][3:] = self.obj_geom_range[
                        "high"
                    ]  # bounding box, (center, size)
                    self.mj_model.geom_rbound[gid] = 2.0 * max(
                        self.obj_geom_range["high"]
                    )  # radius of bounding sphere

                    self.mj_model.geom_pos[gid] = self.np_random.uniform(
                        low=-1.0 * self.mj_model.geom_size[gid],
                        high=self.mj_model.geom_size[gid],
                    )  # random pos
                    self.mj_model.geom_quat[gid] = euler2quat(
                        self.np_random.uniform(
                            low=(-np.pi / 2, -np.pi / 2, -np.pi / 2),
                            high=(np.pi / 2, np.pi / 2, np.pi / 2),
                        )
                    )  # random quat
                    self.mj_model.geom_rgba[gid] = self.np_random.uniform(
                        low=[0.2, 0.2, 0.2, 1], high=[0.9, 0.9, 0.9, 1]
                    )  # random color

                    # friction changes
                    if self.obj_friction_range is not None:
                        self.mj_model.geom_friction[gid] = self.np_random.uniform(
        ...
        if self.qpos_noise_range is not None:
            reset_qpos_local = self.init_qpos + self.qpos_noise_range * (
                self.mj_model.jnt_range[:, 1] - self.mj_model.jnt_range[:, 0]
            )
        ...
        obs = super().reset(
            reset_qpos=reset_qpos_local, reset_qvel=reset_qvel, **kwargs
        )

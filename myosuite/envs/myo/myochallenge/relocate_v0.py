...
class RelocateEnvV0(BaseV0):
    ...
    def reset(self, reset_qpos=None, reset_qvel=None, **kwargs):
        self.sim.model.body_pos[self.goal_bid] = self.np_random.uniform(
        ...
        if self.obj_xyz_range is not None:
            self.sim.model.body_pos[self.object_bid] = self.np_random.uniform(
                **self.obj_xyz_range
            )
        ...
        if self.obj_geom_range is not None:
            for body in [
                ...
                for gid in range(self.sim.model.body_geomnum[bid]):
                    ...
                    self.sim.model.geom_type[gid] = self.np_random.choice(
                        [2, 3, 4, 5, 6]
                    )  # random shape
                    self.sim.model.geom_size[gid] = self.np_random.uniform(
                        low=self.obj_geom_range["low"], high=self.obj_geom_range["high"]
                    )  # random size
                    self.sim.model.geom_aabb[gid][3:] = self.obj_geom_range[
                        "high"
                    ]  # bounding box, (center, size)
                    self.sim.model.geom_rbound[gid] = 2.0 * max(
                        self.obj_geom_range["high"]
                    )  # radius of bounding sphere

                    self.sim.model.geom_pos[gid] = self.np_random.uniform(
                        low=-1.0 * self.sim.model.geom_size[gid],
                        high=self.sim.model.geom_size[gid],
                    )  # random pos
                    self.sim.model.geom_quat[gid] = euler2quat(
                        self.np_random.uniform(
                            low=(-np.pi / 2, -np.pi / 2, -np.pi / 2),
                            high=(np.pi / 2, np.pi / 2, np.pi / 2),
                        )
                    )  # random quat
                    self.sim.model.geom_rgba[gid] = self.np_random.uniform(
                        low=[0.2, 0.2, 0.2, 1], high=[0.9, 0.9, 0.9, 1]
                    )  # random color

                    # friction changes
                    if self.obj_friction_range is not None:
                        self.sim.model.geom_friction[gid] = self.np_random.uniform(

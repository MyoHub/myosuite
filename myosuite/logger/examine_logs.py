...
@click.option(
    "-p", "--rollout_path", type=str, help="absolute path of the rollout", default=None
)
@click.option(
    "-f",
    "--rollout_format",
...
@click.option(
    "-m",
    "--mode",
    type=click.Choice(["record", "render", "playback", "recover"]),
    help="How to examine rollout",
    default="playback",
)
@click.option(
    "-h", "--horizon", type=int, help="Rollout horizon, when mode is record", default=-1
)
@click.option(
    "-s",
    "--seed",
...
@click.option(
    "-n", "--num_repeat", type=int, help="number of repeats for the rollouts", default=1
)
@click.option(
    "-r",
    "--render",
...
@click.option(
    ...
    help=("list of camera names for rendering"),
)
@click.option(
    "-fs",
    "--frame_size",
    type=tuple,
    default=(424, 240),
    help=("Camera frame size for rendering"),
)
@click.option(
...
def examine_logs(
    ...
    if mode == "record":
        ...
        if save_paths is False:
            print(
                "Warning: Recording is not being saved. Enable save_paths=True to log the recorded path"
            )
        paths = [
            None,
        ] * num_repeat  # Mark old traces as None
    else:
        assert rollout_path is not None, "Rollout path is required for mode:{} ".format(
            mode
        )
        ...
        for path_name, path_data in paths.items():
            ...
            if path_data and rollout_format == "RoboHive":
                ...
                if "state" in path_data["env_infos"].keys():
                    path_state = tensor_utils.split_tensor_dict_list(
                        path_data["env_infos"]["state"]
                    )
                    env.reset(
                        reset_qpos=path_state[0]["qpos"],
                        reset_qvel=path_state[0]["qvel"],
                    )
            ...
            trace_horizon = (
                horizon if mode == "record" else path_data["time"].shape[0] - 1
            )
            ...
            for i_step in range(trace_horizon + 1):
                ...
                # Directly create the scene
                    ...
                    if rollout_format == "RoboSet":
                        act = np.concatenate(
                            [
                                path_data["ctrl_arm"][i_step],
                                path_data["ctrl_ee"][i_step],
                            ]
                        )
                    elif (
                        rollout_format == "RoboHive"
                        and "state" in path_data["env_infos"].keys()
                    ):
                ...
                # Recover actions from states
                    ...
                    if rollout_format == "RoboSet":
                        act = np.concatenate(
                            [path_data["qp_arm"][i_step], path_data["qp_ee"][i_step]]
                        )
                    elif rollout_format == "RoboHive":
                        act = path_data["env_infos"]["obs_dict"]["qp"][i_step]
                    if noise_scale:
                        act = act + env.np_random.uniform(
                            high=noise_scale, low=-noise_scale, size=len(act)
                        ).astype(act.dtype)
                ...
                if compress_paths:
                    ...
                    if "state" in env_info.keys():
                        del env_info[
                            "state"
                        ]  # don't save state, obs_dict has env necessities
                ...
                if render == "offscreen":
                    for cam in camera_name:
                        ...
                        # cam_key = cam+'_rgb'
                        trace.append_datum(
                            group_key=path_name, dataset_key=cam, dataset_val=curr_frame
                        )

                # step/forward env using actions from t=>t+1 ----------------------
                if mode == "render" and i_step < trace_horizon:
                    if rollout_format == "RoboSet":
                        env.mj_data.qpos[:nq_arm] = path_data["qp_arm"][i_step + 1]
                        env.mj_data.qpos[nq_arm : nq_arm + nq_ee] = path_data["qp_ee"][
                            i_step + 1
                        ]
                        env.mj_data.qvel[:nq_arm] = path_data["qv_arm"][i_step + 1]
                        env.mj_data.qvel[nq_arm : nq_arm + nq_ee] = path_data["qv_ee"][
                            i_step + 1
                        ]
                        env.mj_data.time = path_data["time"][i_step + 1]
                    elif (
                        rollout_format == "RoboHive"
                        and "state" in path_data["env_infos"].keys()
                    ):
                    ...
                    obs, rwd, done, *_, env_info = env.step(
                        act, update_exteroception=include_exteroception
                    )
                    ep_rwd += rwd
            ...
            # Finish rollout
            old_stat = (
                f"(Old rollout rewards: {sum(path_data['rewards'][:])})"
                if path_data
                else ""
            )
            print(

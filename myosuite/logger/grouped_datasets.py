    ...
    def get_type(input_type):
        ...
        if isinstance(input_type, str):
            if input_type.lower() == "robohive":
                ...
                prompt(
                    f"unknown TraceType{input_type}. Setting it to TraceType.UNSET",
                    type=Prompt.WARN,
                )
...
class Trace:
    ...
    def remove_dataset(self, group_keys: list, dataset_key: str):
        ...
        for group_key in group_keys:
            assert group_key in self.trace.keys(), "Group:{} does not exist".format(
                group_key
            )
    ...
    def append_datum(self, group_key, dataset_key, dataset_val):
        ...
        if dataset_key in self.trace[group_key].keys():
            self.verify_type(
                dataset=self.trace[group_key][dataset_key], data=dataset_val
            )
    ...
    def append_datums(self, group_key: str, dataset_key_val: dict) -> None:
        for dataset_key, dataset_val in dataset_key_val.items():
            self.append_datum(
                group_key=group_key, dataset_key=dataset_key, dataset_val=dataset_val
            )
    ...
    def verify_type(self, dataset, data):
        ...
        assert isinstance(data, dataset_type), TypeError(
            f"Type mismatch while appending. Datum should be {dataset_type}"
        )
        ...
        for grp_k, grp_v in self.trace.items():
            ...
            for i_key, key in enumerate(dataset_keys):
                if i_key == 0:
                    ...
                    assert trace_len == key_len, ValueError(
                        "len({}[{}]={}, should be {}".format(
                            grp_k, key, key_len, trace_len
                        )
                    )
    ...
    def render(
        ...
        for i_grp, grp in enumerate(groups):
            ...
            if output_format == "mp4":
                frames = np.zeros(
                    (horizon, height, width * len(datasets), 3), dtype=np.uint8
                )

            # Render
            print(f"Recovering {output_format} frames:", end="")
            for t in range(horizon):
                # render single frame
                for i_cam, cam_key in enumerate(datasets):
                    frame_tile[:, i_cam * width : (i_cam + 1) * width, :] = self.trace[
                        grp
                    ][cam_key][t]
                ...
                if output_format == "mp4":
                    ...
                    file_name_rgb = os.path.join(
                        output_dir, grp + "-" + str(t) + ".png"
                    )
            ...
            if output_format == "mp4":
                ...
                if platform == "darwin":
                    ...
                    write_video(file_name_mp4, np.asarray(frames), inputdict=inputdict)
                print("\nSaved: " + file_name_mp4)
    ...
    def __repr__(self) -> str:
        disp = f"Trace_name: {self.root.keys()}\n"
            ...
            for grp_k, grp_v in self.trace.items():
                ...
                for dst_k, dst_v in grp_v.items():
                    if isinstance(dst_v, list):
                        ...
                        disp += f"\t{dst_k}: {str(type(dst_v))}\n"
                    else:
                        disp += "\t{}: {}, shape{}, type({})\n".format(
                            dst_k, str(type(dst_v)), dst_v.shape, dst_v.dtype
                        )
                disp += "}\n"
        return disp
    ...
    def numpify(self, u_res, i_res, f_res):
        for grp_k, grp_v in self.trace.items():
            self.trace[grp_k] = dict_numpify(
                data=grp_v, u_res=u_res, i_res=i_res, f_res=f_res
            )

    # Close the logger and post process the data
    def close(
        self, u_res=np.uint8, i_res=np.int8, f_res=np.float16, verify_length=False
    ):
        """

...
class Robot:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.name = (
            robot_name + "(sim)" if is_hardware is None else robot_name + "(hdr)"
        )
        ...
        if self.robot_config is None:
            prompt(
                "Configuring a new session for {}".format(self.name), "white", "on_grey"
            )
            ...
            prompt(
                "Reusing a previours session of {}".format(self.name),
                "white",
                "on_grey",
            )
    ...
    def hardware_init(self, robot_config):
        ...
        for name, device in robot_config.items():
            ...
            if device["interface"]["type"] == "dynamixel":
                ...
                print(
                    "ERROR: interface ({}) not found".format(
        ...
        for name, device in self.robot_config.items():
            if "sensor" in device.keys() and len(device["sensor"]) > 0:
                ...
                for id, sensor in enumerate(device["sensor"]):
                    current_sensor_value[name][id] = (
                        current_sensor_value[name][id] * sensor["scale"]
                        + sensor["offset"]
                    )
    ...
    def get_visual_sensors(
        ...
        if self.is_hardware:
            ...
            for ind, cam_name in enumerate(cameras):
                ...
                ), "{} camera not found".format(cam_name)
    ...
    def sensor2sim(self, sensor, mj_model, mj_data):
        ...
        if not self.is_hardware and (self._noise_scale != 0):
            print(
                "WARNING: Propagating noisy sensors back to sim can destablize simulation"
            )
    ...
    def _advance(self, model, data, n_frames=1, render=None):
    ...
    def step(
        ...
        if self.is_hardware:
            ...
            self._advance(
                self.mj_model, self.mj_data, n_frames, render=(render_cbk != None)
            )
        ...
        if self.is_hardware or realTimeSim:
            ...
            if time_left_in_step > 0.001:
                ...
                prompt(
                    "Step duration %0.4fs, Step took %0.4fs, Time left %0.4f"
                    % (step_duration, (time_now - self.time_wall), time_left_in_step),
                    type=Prompt.WARN,
                )
    ...
    def reset(self, reset_pos, reset_vel, blocking=True, **kwargs):
        ...
        if self.is_hardware:
            ...
            prompt(
                " Done in {}".format(time.time() - t_reset_start),
                "white",
                "on_grey",
                flush=True,
            )
    ...
    def close(self):
        if self.robot_config is not None:
            ...
            if status:
                prompt(
                    f"Closed {self.name} (Status: {status})",
                    "white",
                    "on_grey",
                    flush=True,
                )
                ...
                prompt(
                    f"Error closing {self.name} (Status: {status})",
                    "red",
                    "on_grey",
                    flush=True,
                    type=Prompt.ERROR,

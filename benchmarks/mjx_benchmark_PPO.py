...
def measure_num_env_training_steps(
    meta_seed=0,
    ...
    )  # Overwrite with your env's name
    ...
    def run_benchmark():
        ...
        make_inference_fn, params, _ = ppo.train(
            ...
            save_checkpoint_path=os.path.join(
                os.path.abspath(
                    f"mjx_benchmark_PPO_checkpoints/mjx_benchmark_PPO_{env_name}_{impl}_{num_envs}"
                )
            ),
    ...
    print(
        f"Results for {num_envs} envs: PPO training for {num_timesteps} total steps take {results} seconds."
    )
    res = np.mean(results)
    print(res)
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark PPO training for MyoSuite MJX environments."
    )
    parser.add_argument(
        "--env_name", type=str, default="MjxElbowPoseRandom-v0", help="Environment name"
    )
    parser.add_argument(
        "--impl", type=str, default="warp", help="Backend implementation"
    )
    parser.add_argument(
        "--num_envs", type=int, default=8192, help="Number of environments"
    )
    args = parser.parse_args()

    results = {}
    print(
        f"MyoSuite env {args.env_name} -- Testing implementation: {args.impl} ({args.num_envs} envs)"
    )
    results[f"{args.env_name}_{args.impl}_{args.num_envs}"] = (
        measure_num_env_training_steps(
            meta_seed=0, env_name=args.env_name, impl=args.impl, num_envs=args.num_envs
        )
    )
    print(results)

    np.save(
        f"mjx_benchmark_PPO_results_{args.env_name}_{args.impl}_{args.num_envs}.npy",
        results,
        allow_pickle=True,
    )

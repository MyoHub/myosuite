...
def measure_num_env_simulation_steps(
    num_steps=8192 * 16, env_name="ElbowPoseRandom-v0"
):
    ...
    def single_step():
        a = np.random.uniform(
            low=0.0, high=1.0, size=env.action_space.shape
        )  # Sample a random action
        next_o, r, done, *_, ifo = env.step(a)  # take an action

    results = timeit.repeat(single_step, number=num_steps, repeat=3)
    print(
        f"Results for single CPU env: {num_steps} total steps take {results} seconds."
    )
    res = np.mean(results)
    print(res)
...
def main():
    ...
    assert all(
        [gym.spec(env_name) is not None for env_name in env_names]
    ), "Please ensure the env names are correct and registered in MyoSuite."

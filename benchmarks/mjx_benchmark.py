...
def measure_num_env_simulation_steps(
    seed=0, loop_iterations=16, env_name="MjxElbowPoseRandom-v0", impl="jax"
):
    ...
    for e in [64, 512, 1024, 2048, 4096, 8192]:
        ...
        print(
            f"Results for {e} envs: {8192*loop_iterations} total steps take {results} seconds."
        )
...
def main():
    ...
    for env_name in [
        ...
        for impl in ["warp", "jax"]:
            ...
            results[f"{env_name}_{impl}"] = measure_num_env_simulation_steps(
                seed=0, env_name=env_name, impl=impl
            )
    print(results)
